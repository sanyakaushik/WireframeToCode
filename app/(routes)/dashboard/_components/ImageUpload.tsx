...
function ImageUpload() {
    ...
    const OnImageSelect = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (files) {
            console.log(files[0])
            const imageUrl = URL.createObjectURL(files[0]); {/* We are adding this to covert the selected image into an object so that it gets display on the screen */}
            setFile(files[0]);
            setPreviewUrl(imageUrl);
        }
    }
//for this function OnConverToCodeButtonClick we are using firebase storage 
    const OnConverToCodeButtonClick = async () => {
        ...
        const imageRef = ref(storage, "WireFrameToCode/" + fileName);
    ...
    return (
        <div className='mt-10'>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-10'>{/* style-grid,for larger screens-the 2 divs wil be side by side but for medium it will be vertically */}
                ...
                '>
                    ...
                    <div className='p-5 border border-dashed w-full flex mt-4 justify-center'>
                        <label htmlFor='imageSelect'> {/* here it will connectv to the below input tag.It just helps us in labeling the button */}
                    ...
                    <div className='p-5 border border-dashed'>
                        ...
                        /> {/* this is a X tag means closing the uploaded image and getting back to the original screen */}

                    </div>
                } {/* Here we are running a if-else conditional for whether the image is selected or not . If the image is selected then we will show thw image and hide the upload thing and vice versa */}
